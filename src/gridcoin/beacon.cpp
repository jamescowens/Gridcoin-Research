    ...
    const int64_t superblock_time, const uint256& block_hash, const int& height)
{
    ...
    for (const auto& id : beacon_ids) {
        ...
        if (iter_pair != m_pending.end()) {
            ...
            LogPrintf("INFO: %s: Check post activated beacon insert: beacon class is %s for cpid %s.",
                      __func__,
                      m_beacons[activated_beacon.m_cpid]->m_beacon_class,
                      m_beacons[activated_beacon.m_cpid]->m_cpid.ToString());
