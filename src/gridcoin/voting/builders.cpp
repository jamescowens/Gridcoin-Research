...
public:
    ...
    PollEligibilityClaim BuildClaim(const Poll& poll) const EXCLUSIVE_LOCKS_REQUIRED(cs_main)
    {
        ...
        // Upfront balance check: bail out before AvailableCoins if the wallet
        // obviously cannot satisfy the threshold. The v2 path is dust-resistant
        // by construction (global descending sort + MAX_OUTPOINTS cap), but a
        // total-balance shortfall is a clearer signal than "top N UTXOs sum
        // to less than threshold."
        if (m_wallet.GetBalance() < POLL_REQUIRED_BALANCE) {
            throw VotingError(strprintf(
                _("Wallet balance of %s GRC is less than the required %s GRC to create a poll."),
                FormatMoney(m_wallet.GetBalance()),
...
private:
    ...
    //!
    //!   1. Up-front total-balance check. If the wallet's overall balance is
    //!      below the threshold, no point grouping by address.
    //!   2. Group available outputs by address, recording each address's total
    //!      balance.
    //!   3. Filter to candidate addresses (per-address total >= threshold).
    //!      Without this filter, the post-sort walk in step 5 might include
    //!      every address; with it, the loop only runs over plausible
    //!      candidates.
    //!   4. Sort candidates by total balance DESCENDING. Larger-balance
    //!      addresses are most likely to reach the threshold in the fewest
    //!      outpoints, so try them first.
    //!   5. For each candidate, sort its outpoints by value DESCENDING and
    ...
    //!   6. If no candidate succeeds, the wallet is fragmented enough that
    ...
    {
        // 1. Up-front total balance check
        if (m_wallet.GetBalance() < POLL_REQUIRED_BALANCE) {
            throw VotingError(strprintf(
                _("Wallet balance of %s GRC is less than the required %s GRC to create a poll."),
                FormatMoney(m_wallet.GetBalance()),
        ...
        // 2. Group outputs by address, populating both m_outpoints and m_amounts
        ...
        // 3. Filter to candidates: per-address total >= POLL_REQUIRED_BALANCE.
        ...
        // 4. Sort candidate addresses descending by total balance.
        std::sort(candidates.begin(), candidates.end(), std::greater<AddressOutputs>());

        // 5. For each candidate, sort its outpoints by amount descending and
        ...
        // 6. All candidates exhausted: heavily dust-fragmented within each
...
//! requirement.
//! \return A CCoinControl with the selected outpoints pinned.
//! \throws VotingError if no selection can cover \p needed.
    ...
    const std::optional<COutPoint>& explicit_outpoint)
{
    ...
    // Default: accumulate largest-first until covered.
    std::vector<COutput> available;
    wallet.AvailableCoins(available, true, nullptr, false);
